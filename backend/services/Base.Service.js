...
/**
 * Obtiene un registro específico por un campo y su valor, con la posibilidad de incluir relaciones.
 *
 * @param {Object} Model - El modelo de Sequelize del que se obtendrá el registro.
 * @param {string} field - El nombre del campo por el cual buscar.
 * @param {any} value - El valor del campo a buscar.
 * @param {Array} [include=[]] - Un array de asociaciones para incluir en la consulta (opcional).
 * @returns {Promise<Object|null>} El registro encontrado o `null` si no se encuentra.
 * @throws {Error} Si ocurre un error durante la obtención del registro.
 */
const findByField = async (Model, field, value, include = []) => {
  try {
    ...
    throw new Error(`Error fetching record by field: ${error.message}`);
  }
};
